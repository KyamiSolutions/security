...
module.exports = {
  ...
  pollCurrent: (eWeLinkDevice, feature) => {
    const { deviceId } = parseExternalId(feature.external_id);
    const currentAmperage =
      (eWeLinkDevice.params && Number.parseFloat(eWeLinkDevice.params.current) / POWR3_DIVISOR) || false;
    // if the value is different from the value we have, save new state
    if (currentAmperage && feature.last_value !== currentAmperage) {
      logger.debug(`eWeLink: Polling device "${deviceId}", current new value = ${currentAmperage}`);
